...
// the numbers in even position in the array.
...
// sum = N*(N+1)/2
// but in this case the sum is just for even positions, so can't be used.
...
func main() {
	...
	// Computation of sum of the values in even positions
	var sum int
	for j, n := range a {
		if j%2 == 0 {
			sum += n
